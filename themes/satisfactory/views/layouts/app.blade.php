...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@600;700&family=Exo+2:wght@500;600;700&display=swap" rel="stylesheet">
    ...
    <link rel="stylesheet" href="{{ asset('css/theme-satisfactory.css') }}?v=1">
    @include('theme::partials.theme-mode-boot')
    <style>
        :root {
            --theme-primary: {{ $themeColors['primary'] ?? '#fa9549' }};
            --theme-accent: {{ $themeColors['accent'] ?? '#4a90d9' }};
            --theme-bg: {{ $themeColors['background'] ?? '#0b0d10' }};
            --theme-surface: {{ $themeColors['surface'] ?? '#151a20' }};
            --theme-text: {{ $themeColors['text'] ?? '#f0f3f7' }};
            --theme-muted: {{ $themeColors['text_muted'] ?? '#9aa3ad' }};
        }
    </style>
    {!! app(\App\Support\PluginManager::class)->getHeadHtml() !!}
</head>
<body class="theme-satisfactory theme-shell-lite antialiased">
