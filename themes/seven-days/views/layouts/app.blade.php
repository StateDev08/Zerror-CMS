...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/theme-seven-days.css') }}?v=1">
    @include('theme::partials.theme-mode-boot')
    <style>
        :root {
            --theme-primary: {{ $themeColors['primary'] ?? '#c44b2b' }};
            --theme-accent: {{ $themeColors['accent'] ?? '#8b9a6b' }};
            --theme-bg: {{ $themeColors['background'] ?? '#0e0b0a' }};
            --theme-surface: {{ $themeColors['surface'] ?? '#1a1412' }};
            --theme-text: {{ $themeColors['text'] ?? '#f2e8e0' }};
            --theme-muted: {{ $themeColors['text_muted'] ?? '#b5a396' }};
        }
    </style>
    {!! app(\App\Support\PluginManager::class)->getHeadHtml() !!}
</head>
<body class="theme-seven-days theme-shell-lite antialiased">
