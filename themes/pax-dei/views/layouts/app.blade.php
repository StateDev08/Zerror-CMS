...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/theme-pax-dei.css') }}?v=1">
    @include('theme::partials.theme-mode-boot')
    <style>
        :root {
            --theme-primary: {{ $themeColors['primary'] ?? '#d4af37' }};
            --theme-accent: {{ $themeColors['accent'] ?? '#2f5fd0' }};
            --theme-bg: {{ $themeColors['background'] ?? '#07080f' }};
            --theme-surface: {{ $themeColors['surface'] ?? '#10131c' }};
            --theme-text: {{ $themeColors['text'] ?? '#f7f3e8' }};
            --theme-muted: {{ $themeColors['text_muted'] ?? '#a9b0c3' }};
        }
    </style>
    {!! app(\App\Support\PluginManager::class)->getHeadHtml() !!}
</head>
<body class="theme-pax-dei theme-shell-lite antialiased">
