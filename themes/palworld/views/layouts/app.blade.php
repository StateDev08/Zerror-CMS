...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/theme-palworld.css') }}?v=1">
    @include('theme::partials.theme-mode-boot')
    <style>
        :root {
            --theme-primary: {{ $themeColors['primary'] ?? '#4fc3f7' }};
            --theme-accent: {{ $themeColors['accent'] ?? '#ff8a65' }};
            --theme-bg: {{ $themeColors['background'] ?? '#0a1218' }};
            --theme-surface: {{ $themeColors['surface'] ?? '#12202a' }};
            --theme-text: {{ $themeColors['text'] ?? '#eef9ff' }};
            --theme-muted: {{ $themeColors['text_muted'] ?? '#8eb4c9' }};
        }
    </style>
    {!! app(\App\Support\PluginManager::class)->getHeadHtml() !!}
</head>
<body class="theme-palworld theme-shell-lite antialiased">
