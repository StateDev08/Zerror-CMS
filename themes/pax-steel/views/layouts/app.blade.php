...
<head>
    ...
    <style>
        ...
        .card { border-radius: var(--radius-2xl); border: 1px solid rgb(64 64 64 / 0.9); background: var(--theme-surface); padding: 1.5rem; box-shadow: var(--shadow-md); transition: box-shadow 0.2s ease; color: var(--theme-text); }
        .card:hover { box-shadow: var(--shadow-lg); }
        .btn-primary { display: inline-flex; align-items: center; justify-content: center; padding: 0.75rem 1.25rem; border-radius: var(--radius-xl); background: linear-gradient(180deg, var(--theme-primary), var(--theme-accent)); color: white; font-weight: 600; font-size: 0.9375rem; transition: filter 0.2s ease, box-shadow 0.2s ease; box-shadow: 0 2px 8px rgb(0 0 0 / 0.3); }
        .btn-primary:hover { filter: brightness(1.1); box-shadow: 0 4px 12px rgb(0 0 0 / 0.4); }
        .form-input { width: 100%; border-radius: var(--radius-xl); border: 1px solid rgb(64 64 64); background: var(--theme-surface); color: var(--theme-text); padding: 0.625rem 1rem; font-size: 1rem; transition: border-color 0.2s ease, box-shadow 0.2s ease; }
        .form-input:focus { outline: none; border-color: var(--theme-primary); box-shadow: 0 0 0 3px rgb(100 116 139 / 0.2); }
        ...
        .widget { border-radius: var(--radius-xl); border: 1px solid rgb(64 64 64 / 0.9); background: var(--theme-surface); box-shadow: var(--shadow-sm); transition: box-shadow 0.2s ease; }
...
<body class="min-h-screen antialiased">
    ...
    <header class="md:hidden sticky top-0 z-30 py-3 px-4 flex items-center justify-between rounded-b-2xl shadow-lg border-b border-neutral-600/40 backdrop-blur-md bg-neutral-900/95">
        <button type="button" id="sidebar-toggle" class="p-2.5 rounded-xl border border-neutral-600/50 hover:bg-neutral-700/40 transition-colors" aria-expanded="false" aria-controls="site-sidebar" aria-label="{{ __('nav.menu_toggle') }}">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
        </button>
        <span class="font-semibold text-lg truncate">{{ $siteName }}</span>
        <button type="button" id="theme-mode-toggle" class="p-2.5 rounded-xl border border-neutral-600/50 hover:bg-neutral-700/40 transition-colors shrink-0" aria-label="{{ __('theme.dark_mode_toggle') ?? 'Dark/Light' }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>
        </button>
    </header>
    <div class="flex min-h-screen {{ $navRight ? 'flex-row-reverse' : '' }}">
        <aside id="site-sidebar" data-sidebar-close-class="{{ $navRight ? 'translate-x-full' : '-translate-x-full' }}" class="fixed md:sticky top-0 z-40 h-screen w-72 shrink-0 flex flex-col transition-all duration-300 ease-out {{ $navRight ? 'right-0 md:translate-x-0 translate-x-full' : 'left-0 -translate-x-full md:translate-x-0' }} md:my-4 {{ $navRight ? 'md:mr-4' : 'md:ml-4' }} md:rounded-2xl md:shadow-xl md:h-[calc(100vh-2rem)] border border-neutral-600/40 bg-neutral-900/95">
            <div class="p-5 border-b border-neutral-600/40 flex items-center justify-between gap-2">
                <a href="{{ url('/') }}" class="font-semibold text-lg flex items-center gap-2 min-w-0 text-neutral-100">
                ...
                <button type="button" id="theme-mode-toggle-sidebar" class="p-2.5 rounded-xl border border-neutral-600/50 hover:bg-neutral-700/40 transition-colors shrink-0" aria-label="{{ __('theme.dark_mode_toggle') ?? 'Dark/Light' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>
                </button>
            </div>
            <nav class="flex-1 overflow-y-auto py-4 px-2">
                ...
                @if($leftMenuItems > 0)
                    ...
                    <ul class="flex flex-col gap-1 px-2 mt-4 pt-4 border-t border-neutral-600/40">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <main class="flex-1 container mx-auto px-4 md:px-6 py-8 max-w-7xl flex gap-8 {{ $mainOrderWidgetsFirst ? 'flex-row-reverse' : '' }}">
                ...
                <aside class="w-80 shrink-0 hidden lg:block space-y-5">
                    ...
                    @if($rightMenuItems->isNotEmpty())
                        <div class="rounded-2xl border border-neutral-600/40 bg-neutral-900/80 p-4 shadow-md">
                            <ul class="space-y-1">
            ...
            <footer class="mt-auto py-6 rounded-t-2xl border-t border-neutral-600/40 backdrop-blur-md shadow-[0_-4px_20px_-4px_rgba(0,0,0,0.3)] bg-neutral-900/95">
                <div class="container mx-auto px-4 max-w-7xl text-sm text-neutral-400">
                    ...
                    @if($contactAddress || $contactEmail || $contactPhone || $socialDiscord || $socialFacebook || $socialTwitter || $socialYoutube)
                        <div class="mt-4 pt-4 border-t border-neutral-600/40 flex flex-wrap justify-center gap-x-5 gap-y-1">
    ...
    @unless(cookie('zerrocms_cookie_consent'))
        <div id="cookie-consent" class="fixed bottom-0 left-0 right-0 px-6 py-4 text-center text-sm z-50 flex flex-wrap items-center justify-center gap-3 rounded-t-2xl backdrop-blur-md border-t border-neutral-600/40 shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.3)] bg-neutral-900/98 text-neutral-200">
