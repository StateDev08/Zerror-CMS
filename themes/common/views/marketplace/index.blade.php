...
@section('title', __('nav.marketplace') . ' - ' . site_name())

@section('content')
    <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
        <h1 class="text-2xl font-bold">{{ __('nav.marketplace') }}</h1>
        @auth
            <a href="{{ route('marketplace.create') }}" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded">{{ __('marketplace.create') }}</a>
        @endauth
    </div>
    <section class="mb-8">
