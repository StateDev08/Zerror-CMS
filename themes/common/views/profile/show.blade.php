...
@section('title', ($profileUser->name ?? __('usercp.profile')) . ' - ' . site_name())
    ...
    <div class="rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-6 max-w-2xl">
        ...
        @if($profileUser->biography)
            <div class="mt-6">
                ...
                <div class="text-gray-600 dark:text-gray-400">{{ html_content($profileUser->biography) }}</div>
            </div>
        @endif
        @if($profileUser->about_me)
            <div class="mt-6">
                <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">{{ __('usercp.about_me') }}</h2>
                <div class="text-gray-600 dark:text-gray-400">{{ html_content($profileUser->about_me) }}</div>
            </div>
        @endif
