...
@section('title', __('crafting.create_title') . ' - ' . site_name())
...
@section('content')
    ...
    <div class="card max-w-xl">
        <form action="{{ route('crafting.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">{{ __('crafting.custom_request') }}</label>
                <livewire:cms-rich-editor-field name="custom_request" :value="old('custom_request')" :compact="true" />
                @error('custom_request')<p class="text-red-600 dark:text-red-400 text-sm mt-1.5">{{ $message }}</p>@enderror
            </div>
