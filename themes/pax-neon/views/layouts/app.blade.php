...
<head>
    ...
    <style>
        :root {
            ...
            --shadow-lg: 0 10px 15px -3px rgb(0 0 0 / 0.2); --shadow-xl: 0 20px 25px -5px rgb(0 0 0 / 0.2);
            --glow-primary: 0 0 20px color-mix(in srgb, var(--theme-primary) 40%, transparent);
        }
        ...
        .theme-bg-primary { background: linear-gradient(180deg, var(--theme-primary), var(--theme-accent)); }
        .theme-bg-accent { background-color: var(--theme-accent); }
        #site-sidebar .theme-nav-active { border-left-color: var(--theme-primary); color: var(--theme-primary); font-weight: 600; box-shadow: inset 0 0 12px color-mix(in srgb, var(--theme-primary) 15%, transparent); }
        .page-title { font-size: 1.75rem; font-weight: 700; color: var(--theme-text); letter-spacing: -0.025em; }
        .card { border-radius: var(--radius-2xl); border: 1px solid color-mix(in srgb, var(--theme-primary) 25%, var(--theme-surface)); background: var(--theme-surface); padding: 1.5rem; box-shadow: var(--shadow-md); transition: box-shadow 0.2s ease; color: var(--theme-text); }
        .card:hover { box-shadow: var(--shadow-lg), var(--glow-primary); }
        .btn-primary { display: inline-flex; align-items: center; justify-content: center; padding: 0.75rem 1.25rem; border-radius: var(--radius-xl); background: linear-gradient(180deg, var(--theme-primary), var(--theme-accent)); color: #0a0a0f; font-weight: 600; font-size: 0.9375rem; transition: filter 0.2s ease, box-shadow 0.2s ease; box-shadow: var(--glow-primary); }
        .btn-primary:hover { filter: brightness(1.1); box-shadow: 0 0 24px color-mix(in srgb, var(--theme-primary) 50%, transparent); }
        .form-input { width: 100%; border-radius: var(--radius-xl); border: 1px solid color-mix(in srgb, var(--theme-primary) 30%, transparent); background: var(--theme-surface); color: var(--theme-text); padding: 0.625rem 1rem; font-size: 1rem; transition: border-color 0.2s ease, box-shadow 0.2s ease; }
        ...
        .alert-success { padding: 1rem 1.25rem; border-radius: var(--radius-xl); background: color-mix(in srgb, var(--theme-primary) 15%, var(--theme-surface)); color: var(--theme-primary); border: 1px solid color-mix(in srgb, var(--theme-primary) 40%, transparent); }
        .alert-warning { padding: 1rem 1.25rem; border-radius: var(--radius-xl); background: color-mix(in srgb, #f59e0b 15%, var(--theme-surface)); color: #fbbf24; border: 1px solid color-mix(in srgb, #f59e0b 40%, transparent); }
        .widget { border-radius: var(--radius-xl); border: 1px solid color-mix(in srgb, var(--theme-primary) 20%, var(--theme-surface)); background: var(--theme-surface); box-shadow: var(--shadow-sm); transition: box-shadow 0.2s ease; }
        .widget:hover { box-shadow: var(--shadow-md), var(--glow-primary); }
...
<body class="min-h-screen antialiased">
    ...
    <header class="md:hidden sticky top-0 z-30 py-3 px-4 flex items-center justify-between rounded-b-2xl shadow-lg border-b border-white/10 backdrop-blur-md" style="background: color-mix(in srgb, var(--theme-surface) 95%, transparent);">
        <button type="button" id="sidebar-toggle" class="p-2.5 rounded-xl border border-white/20 hover:bg-white/10 transition-colors" aria-expanded="false" aria-controls="site-sidebar" aria-label="{{ __('nav.menu_toggle') }}">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
        </button>
        <span class="font-semibold text-lg truncate">{{ $siteName }}</span>
        <button type="button" id="theme-mode-toggle" class="p-2.5 rounded-xl border border-white/20 hover:bg-white/10 transition-colors shrink-0" aria-label="{{ __('theme.dark_mode_toggle') ?? 'Dark/Light' }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>
        </button>
    </header>
    <div class="flex min-h-screen {{ $navRight ? 'flex-row-reverse' : '' }}">
        <aside id="site-sidebar" data-sidebar-close-class="{{ $navRight ? 'translate-x-full' : '-translate-x-full' }}" class="fixed md:sticky top-0 z-40 h-screen w-72 shrink-0 flex flex-col transition-all duration-300 ease-out {{ $navRight ? 'right-0 md:translate-x-0 translate-x-full' : 'left-0 -translate-x-full md:translate-x-0' }} md:my-4 {{ $navRight ? 'md:mr-4' : 'md:ml-4' }} md:rounded-2xl md:shadow-xl md:h-[calc(100vh-2rem)] border border-white/10" style="background: var(--theme-surface);">
            <div class="p-5 border-b border-white/10 flex items-center justify-between gap-2">
                <a href="{{ url('/') }}" class="font-semibold text-lg flex items-center gap-2 min-w-0" style="color: var(--theme-text);">
                ...
                <button type="button" id="theme-mode-toggle-sidebar" class="p-2.5 rounded-xl border border-white/20 hover:bg-white/10 transition-colors shrink-0" aria-label="{{ __('theme.dark_mode_toggle') ?? 'Dark/Light' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>
                </button>
            </div>
            <nav class="flex-1 overflow-y-auto py-4 px-2">
                ...
                @if($leftMenuItems > 0)
                    ...
                    <ul class="flex flex-col gap-1 px-2 mt-4 pt-4 border-t border-white/10">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <main class="flex-1 container mx-auto px-4 md:px-6 py-8 max-w-7xl flex gap-8 {{ $mainOrderWidgetsFirst ? 'flex-row-reverse' : '' }}">
                ...
                <aside class="w-80 shrink-0 hidden lg:block space-y-5">
                    ...
                    @if($rightMenuItems->isNotEmpty())
                        <div class="rounded-2xl border border-white/10 p-4 shadow-md" style="background: var(--theme-surface);">
                            <ul class="space-y-1">
            ...
            <footer class="mt-auto py-6 rounded-t-2xl border-t border-white/10 backdrop-blur-md shadow-[0_-4px_20px_-4px_rgba(0,0,0,0.3)]" style="background: color-mix(in srgb, var(--theme-surface) 95%, transparent);">
                <div class="container mx-auto px-4 max-w-7xl text-sm" style="color: var(--theme-text-muted);">
                    ...
                    @if($contactAddress || $contactEmail || $contactPhone || $socialDiscord || $socialFacebook || $socialTwitter || $socialYoutube)
                        <div class="mt-4 pt-4 border-t border-white/10 flex flex-wrap justify-center gap-x-5 gap-y-1">
    ...
    @unless(cookie('zerrocms_cookie_consent'))
        <div id="cookie-consent" class="fixed bottom-0 left-0 right-0 px-6 py-4 text-center text-sm z-50 flex flex-wrap items-center justify-center gap-3 rounded-t-2xl backdrop-blur-md border-t border-white/10 shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.3)]" style="background: color-mix(in srgb, var(--theme-surface) 98%, transparent); color: var(--theme-text);">
            <span>{{ __('cookies.banner_text') }}</span>
            <a href="{{ route('page.show', ['slug' => 'cookies']) }}" class="theme-link-primary underline">{{ __('nav.cookies') }}</a>
            <button type="button" onclick="document.cookie='zerrocms_cookie_consent=1;path=/;max-age=31536000'; document.getElementById('cookie-consent').remove();" class="px-4 py-2 rounded-xl font-medium transition-all theme-bg-primary text-[#0a0a0f] hover:brightness-110">{{ __('cookies.accept') }}</button>
        </div>
    @endunless
    <script>
